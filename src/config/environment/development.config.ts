export default () => ({
  app: {
    ...
    baseUrl: process.env.BASE_URL || `http://localhost:${process.env.PORT || '8088'}`,
    apiUrl: process.env.API_URL || `http://localhost:${process.env.PORT || '8088'}`,
  },
  urls: {
  ...
  database: {
    url: process.env.DATABASE_URL || 'postgresql://postgres:postgres@postgres:5432/userdb?schema=public',
  },
  redis: {
